...
export const generateComprehensiveReportPDF = (
  ...
  doc.setFontSize(20);
  doc.setTextColor(41, 98, 255);
  doc.text("COMPREHENSIVE ACCOUNT REPORT", 14, 20);
  ...
  doc.setTextColor(0, 0, 0);
  doc.text(`Statement Period: ${format(new Date(fromDate), 'dd MMM yyyy')} to ${format(new Date(toDate), 'dd MMM yyyy')}`, 14, 65);

  // Calculate Opening Balance
  const pastEntries = entries.filter(e => new Date(e.date) < new Date(fromDate));
  const pastPayments = payments.filter(p => new Date(p.date) < new Date(fromDate));
  ...
  const periodEntries = entries.filter(e => new Date(e.date) >= new Date(fromDate) && new Date(e.date) <= new Date(toDate + 'T23:59:59'));
  const periodPayments = payments.filter(p => new Date(p.date) >= new Date(fromDate) && new Date(p.date) <= new Date(toDate + 'T23:59:59'));

  // Build Ledger
  const ledger: { date: Date, desc: string, debit: number, credit: number }[] = [];
  
  periodEntries.forEach(e => ledger.push({ date: new Date(e.date), desc: `Trip - ${e.type}`, debit: e.price, credit: 0 }));
  periodPayments.forEach(p => ledger.push({ date: new Date(p.date), desc: `Payment (${p.mode})`, debit: 0, credit: p.amount }));

  ledger.sort((a, b) => a.date.getTime() - b.date.getTime());

  const tableData = [
    [format(new Date(fromDate), 'dd-MM-yyyy'), 'Opening Balance', '-', '-', formatNumber(balance)]
  ];
  ...
  autoTable(doc, {
    startY: 75,
    head: [['Date', 'Particulars', 'Billed (Dr)', 'Received (Cr)', 'Balance (Rs)']],
    body: tableData,
    theme: 'grid',
    headStyles: { fillColor: [41, 98, 255] },
    footStyles: { fillColor: [240, 240, 240], textColor: [0, 0, 0], fontStyle: 'bold' }
  ...
  doc.text(`Closing Balance as of ${format(new Date(toDate), 'dd MMM yyyy')}: Rs. ${formatNumber(balance)}`, 14, finalY1);
  ...
  doc.setFontSize(18);
  doc.setTextColor(41, 98, 255);
  doc.text("AUTOMATED BILLING SUMMARY", 14, 20);
  ...
  autoTable(doc, {
    ...
    body: billsData.length > 0 ? billsData : [['No billing periods found', '', '', '']],
    theme: 'grid',
    headStyles: { fillColor: [41, 98, 255] },
  });
  ...
  autoTable(doc, {
    ...
    body: txData.length > 0 ? txData : [['No recent transactions', '', '', '', '']],
    theme: 'grid',
